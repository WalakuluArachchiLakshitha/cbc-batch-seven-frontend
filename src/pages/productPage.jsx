...
export function ProductPage() {
    ...
    return (
        <div className="w-full min-h-[calc(100vh-100px)] bg-primary flex flex-col items-center pt-8">
            <Helmet>
                <title>Shop All Products - Cosmetic Shop</title>
                <meta name="description" content="Browse our extensive collection of premium cosmetic products." />
            </Helmet>
            {isLoading ? (
                <Loader />
            ) : (
                <>
                    ...
                    <div className="w-full max-w-7xl px-4 mb-8">
                        <h1 className="text-3xl font-bold text-secondary mb-6 text-center">Our Collection</h1>
                        <div className="flex flex-wrap justify-center gap-3">
                            {categories.map((category) => (
                                <button
                                    key={category}
                                    onClick={() => setSelectedCategory(category)}
                                    className={`px-5 py-2 rounded-full text-sm font-semibold transition-all border ${selectedCategory === category
