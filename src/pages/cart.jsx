      ...
      <div className="w-full max-w-[600px] px-4 flex flex-col gap-4">
        {cart.map((item, index) => {
          return (
            <div
              key={index}
              className="w-full bg-white flex flex-col lg:flex-row relative items-center p-4 rounded-xl shadow-sm border border-secondary/10"
            >
              <button
                className="absolute top-2 right-2 lg:top-1/2 lg:-right-12 lg:-translate-y-1/2 text-red-500 text-xl lg:text-2xl rounded-full p-2 hover:bg-red-50 transition-colors"
                onClick={() => {
              ...
              <img
                className="h-[100px] w-[100px] lg:h-full lg:w-[120px] object-cover rounded-lg"
                src={item.image}
              />
              ...
              <div className="w-full lg:w-auto flex flex-row lg:flex-col justify-center items-center gap-3 my-3 lg:my-0 lg:ml-auto">
                <CiCircleChevUp
                  className="text-3xl text-secondary hover:text-accent cursor-pointer transition-colors"
                  onClick={() => {
                    addToCart(item, 1);
                    setCart(loadCart());
                  }}
        ...
        {cart.length > 0 && (
          <div className="w-full bg-white p-4 rounded-xl shadow-sm border border-secondary/10 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <Link
              state={cart}
              to="/checkout"
              className="w-full md:w-auto bg-accent text-white px-8 py-3 rounded-xl font-bold hover:bg-accent/80 transition-all text-center"
            >
              Proceed to Checkout
            </Link>
          </div>
