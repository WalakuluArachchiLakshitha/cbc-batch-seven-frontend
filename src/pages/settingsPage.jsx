...
export default function UserSettings() {
  ...
  async function updateUserData() {
    try {
      let imageLink = user?.image || "";
      if (image) imageLink = await mediaUpload(image);

      const data = { firstName, lastName, image: imageLink };
  ...
  return (
    <div className="min-h-screen w-full bg-[url('/bg.jpg')] bg-cover bg-center bg-no-repeat flex flex-col items-center py-6 overflow-auto">
      {/* Container */}
      <div className="w-full max-w-3xl flex flex-col lg:flex-row justify-center gap-6 px-4">
        {/* User Info */}
        <div className="w-full lg:w-1/2 backdrop-blur-2xl rounded-2xl p-6 flex flex-col bg-primary/80 shadow-xl ring-1 ring-secondary/20">
          <h1 className="text-2xl font-bold mb-6 text-center text-secondary">
            User Settings
          </h1>
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <label className="inline-flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer bg-white/70 hover:bg-white transition border border-secondary/10 text-sm">
              Upload Photo
              <input
                type="file"
                accept="image/*"
